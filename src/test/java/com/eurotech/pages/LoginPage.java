...
public class LoginPage extends BasePage{
    ...
    //@FindAll
    //To use multiple locator if at least one of them is matching it will find the web element.     ONE MATCHES OK !!!
    @FindAll({
            @FindBy(id = "loginpage-input-email"), //or  --> logical OR
            @FindBy(name = "emailhghjghjghghghjgjh")})
    public WebElement usernameInputFindAll;


    //@FindBys
    //if the locators are matching with the element then it will return it.    BOTH SHOULD MATCH !!!
    @FindBys({
            @FindBy(id = "loginpage-form-pw-input"),   // and   --> logical AND
